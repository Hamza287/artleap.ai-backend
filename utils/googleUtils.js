...
const mapGoogleProductType = (sku) => {
  try {
    ...
    throw new Error(`Failed to map Google product type: ${error.message}`);
  }
};
...
const mapBillingPeriod = (sku) => {
  try {
    ...
    throw new Error(`Failed to map billing period: ${error.message}`);
  }
};
...
const parseFeatures = (description) => {
  try {
    ...
    throw new Error(`Failed to parse features: ${error.message}`);
  }
};
